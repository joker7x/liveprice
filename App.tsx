          ...
          >
              ...
              <div className="relative max-w-md mx-auto px-5 pt-14 pb-4">
                <div className="flex items-center justify-between mb-5">
                  ...
                  <motion.div 
                    ...
                    className={`px-3 py-1.5 rounded-full text-[10px] font-bold flex items-center gap-1.5 border backdrop-blur-sm ${isOffline ? 'bg-red-50/80 text-red-600 border-red-200' : 'bg-emerald-50/80 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20'}`}
                  >
                ...
                >
                  ...
                  <input
                    ...
                    className={`w-full backdrop-blur-xl pl-4 pr-11 py-3.5 rounded-[20px] border shadow-[0_4px_20px_rgba(0,0,0,0.03)] focus:outline-none focus:ring-[2px] transition-all duration-300 text-[15px] font-medium
                      ${darkMode 
            ...
            <AnimatePresence>
                {broadcast?.isActive && (
                    ...
                    >
                        <div className={`mt-2 p-3 rounded-2xl flex items-start gap-3 border ${broadcast.type === 'warning' ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-500/20 text-amber-800 dark:text-amber-100' : 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-500/20 text-blue-800 dark:text-blue-100'}`}>
            ...
            <main className="max-w-md mx-auto pt-2 relative z-10">
              ...
              <div className="px-5 space-y-4 mt-2">
                {drugs.length === 0 && loading ? (
                  ...
                  >
                    <div className={`w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 text-4xl shadow-sm border ${darkMode ? 'bg-slate-800/50 border-white/5' : 'bg-white/50 border-white'}`}>
                      🔍
                    </div>
                ...
                {hasMore && filteredDrugs.length > 0 && mode === 'all' && !search && (
                  <div className="pt-8 pb-20 flex justify-center">
                    <motion.button
                      ...
                      disabled={loading}
                      className={`group relative px-8 py-3.5 rounded-full shadow-[0_10px_30px_rgba(37,99,235,0.15)] text-sm font-bold border transition-all disabled:opacity-50 flex items-center gap-3 ${darkMode ? 'bg-slate-800 text-blue-400 border-white/10' : 'bg-white text-blue-600 border-blue-50'}`}
                    >
      ...
      <AnimatePresence>
          {selectedDrug && (
              <DrugIntelligenceModal 
                  ...
                  // onEdit is not passed here, making it read-only for normal users
              />
          )}
      </AnimatePresence>
            ...
            >
              ...
              <form onSubmit={handleAdminAuth}>
                <input 
                  ...
                  className="w-full text-center text-3xl font-mono tracking-[10px] bg-gray-100 dark:bg-slate-900 border-2 border-transparent focus:border-blue-500 rounded-2xl py-3 mb-6 outline-none dark:text-white"
                  placeholder="••••"
                />
                <button 
                  type="submit"
